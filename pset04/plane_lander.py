...
def draw_tree(x, y):
    ...
    pygame.draw.polygon(
        screen, (0, 128, 0), [(x - 30, y - 20), (x + 30, y - 20), (x, y - 100)]
    )
...
def draw_restart():
    pygame.draw.polygon(
        screen, (255, 0, 0), [(10, 10), (350, 10), (350, 100), (10, 100)]
    )
    font.render_to(screen, (20, 20), "REWIND", (255, 255, 255))


def draw_instrument(curr, max, y=0):

    pygame.draw.arc(screen, (250, 50, 0), (50, 50 + y, 150, 150), 0, math.pi)

    rotation = (curr / max * math.pi + math.pi / 2) * -1
    x1 = 125
    y1 = 125 + y
    radius = 100
    x2 = radius * math.sin(rotation) + x1
    y2 = radius * math.cos(rotation) + y1
    x = 50
    y = 50
    rotated = (
    ...
    # speedometer is defined


def draw_speedometer(speed):
    draw_instrument(speed, MAX_PLANE_SPEED)

    # rotate line with altimeter


def draw_altimeter(height):
    draw_instrument(GROUND_LEVEL - height, GROUND_LEVEL, 150)


def draw_landed():
    font.render_to(screen, (WIDTH / 2, HEIGHT / 2), "LANDED", (0, 0, 0))
...
def draw_scene():
    ...
    if plane.state == "crashed":
        # called restart function
        draw_restart()
        # plane has touched ground
    if plane.state == "stopped":
        # create landing sequence
        draw_landed()
    else:
        draw_speedometer(plane.speed)
        draw_altimeter(plane.y)
    clock.tick(60)
    pygame.display.flip()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_DOWN and plane.state == "flying":
                ...
                plane.state = "braking"
            elif event.key == pygame.K_RETURN and plane.state == "crashed":
                # initiate rewind
                plane.state = "rewind"
